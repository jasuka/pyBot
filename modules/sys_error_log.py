...
def log ( self ):
	...
	if os.path.exists(self.config["log-path"]) == True:	#Checking if log-path in config is valid and exists
		...
		logline = "{0}{1}{2} {3}\r\n".format( #0-2 timestamp with brackets, 3 nick, 4 channel, 5 the message
		...
		try:
			...
			if self.config["debug"] == "true":
				print("[ERROR]-[sys_error_log] sys_error_log() stating: {0}, Plus that im retard, and i cant log my own errors :((".format(e))
